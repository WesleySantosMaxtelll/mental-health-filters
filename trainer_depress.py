...
class Trainer():
    ...
    def run_train(self, task, sel, model_name, type_):
        ...
        try:
            ...
            tokenizer = BertTokenizerFast.from_pretrained("pablocosta/bertweet-br-base-uncased")
            X_train, y_train = get_tokens_mask_dates_split(df_train['Text'].tolist(), df_train['label'].tolist(),
                                                           tokenizer, f'{type_}_train', task, sel)
            X_test, y_test = get_tokens_mask_dates_split(df_test['Text'].tolist(), df_test['label'].tolist(), tokenizer,
...
# Trainer().run_train('D', 'relevancia', 'bert_twitter', 'Bert_relevancia_baixo')
Trainer().run_train('A', 'all', 'bert_twitter', 'Bert_LSTM')
