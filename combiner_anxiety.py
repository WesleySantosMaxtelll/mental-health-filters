...
def A_test_with_full_timeline_combiner(rel_al, rel_b, ment):
    try:
        X_text, y_real_label, pred_rel, pred_rel_baixo, pred_mentions = pickle.load(open('A_combiner_test_dl.pkl', 'rb'))
    except:
        from sklearn.model_selection import train_test_split

        df_train = pickle.load(open(f'converted_dataframes/A_train_converted_predictions.pkl', 'rb'))
        df_test = pickle.load(open(f'converted_dataframes/A_test_converted_predictions.pkl', 'rb'))

        pred_mentions = A_prediction_test_mentions()

        df_train['label'] = (df_train['Diagnosed_YN'] == 'yes').astype(int)
        df_test['label'] = (df_test['Diagnosed_YN'] == 'yes').astype(int)

        X_text = df_test['Text'].tolist()
        y_real_label = df_test['label'].tolist()
        pred_rel = df_test['alto_medio_predicao'].tolist()
        pred_rel_baixo = df_test['baixo_predicao'].tolist()
        pickle.dump([X_text, y_real_label, pred_rel, pred_rel_baixo, pred_mentions], open('A_combiner_test_dl.pkl', 'wb'))
    ...
    for epoch in range(0, 50):
        load_checkpoint(f'/home/wesley/Documents/doutorado/doutorado_combinacao_modelos/models/COMBINER-model-A-gating-sel-combiner-epoch{epoch}-{rel_al}-{rel_b}-{ment}-.pt', model)
        with torch.no_grad():
            # validation loop
            model.eval()
            pred, real = [], []
...
def A_train_with_full_timeline_combiner(rel_al, rel_b, ment):
    try:
        X_text, y_real_label, pred_rel, pred_rel_baixo, pred_mentions = pickle.load(open('A_combiner_val_dl.pkl', 'rb'))
    except:
        from sklearn.model_selection import train_test_split
        ...
        pred_mentions = A_prediction_val_mentions()
        ...
        _, X_text, _, y_real_label = train_test_split(df_train['Text'].tolist(), df_train['label'].tolist(), test_size=0.3,
                                                                      random_state=42)
        _, _, _, pred_rel = train_test_split(df_train['Text'].tolist(), df_train['alto_medio_predicao'].tolist(),
                                                      test_size=0.3,
                                                      random_state=42)
        _, _, _, pred_rel_baixo = train_test_split(df_train['Text'].tolist(), df_train['baixo_predicao'].tolist(),
                                                      test_size=0.3,
                                                      random_state=42)
        pickle.dump([X_text, y_real_label, pred_rel, pred_rel_baixo, pred_mentions], open('A_combiner_val_dl.pkl', 'wb'))
    ...
    for epoch in range(epochs):
        ...
        print('Results')
        print(classification_report(true_label, pred))
        save_checkpoint(file_path + '/' + f'COMBINER-model-A-gating-sel-combiner-epoch{epoch}-{rel_al}-{rel_b}-{ment}-.pt',
                        model, 0)
        with torch.no_grad():
            # validation loop
            model.eval()
...
def A_train_bert_mentions_combiner():
    try:
        pred_ment, pred, y_test = pickle.load(open("A_bert_predictions.pkl", "rb"))
    except:
        from sklearn.model_selection import train_test_split
...
def test_with_full_timeline_combiner(rel_al, rel_b, ment, s1to10_5):
    try:
        # 1/0
...
if __name__ == '__main__':
    # combs = [get_tuple(i) for i in [15]]
    # pred_am, real_am = A_test_with_full_timeline_combiner(True, False, False)
    # pred_m, real_m = A_test_with_full_timeline_combiner(False, False, True)
    # pred_b, real_b = A_test_with_full_timeline_combiner(False, True, False)
    #
    # pred_am_b, real_am_b = A_test_with_full_timeline_combiner(True, True, False)
    # pred_b_m, real_b_m = A_test_with_full_timeline_combiner(False, True, True)
    # pred_am_m, real_am_m = A_test_with_full_timeline_combiner(True, False, True)
    # pred_am_b_m, real_am_b_m = A_test_with_full_timeline_combiner(True, True, True)

    combs = [(True, True, True)]
    for c in combs:
        # A_train_with_full_timeline_combiner(*c)
        A_test_with_full_timeline_combiner(*c)
